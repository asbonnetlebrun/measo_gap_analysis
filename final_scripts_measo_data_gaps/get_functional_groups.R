...
get_worms_fgrp <- function(AphiaID){
  ...
  if(!identical(class(attr_dat), "try-error")){
    ...
    if("Functional group" %in% attr_dat$measurementType){
      ...
      if(max(lengths(children)) > 0 ){
        ...
        #' ASBL: added to deal with cases where the same info is recorded for certain specific stages and for stages = NA
        life_stage <- life_stage %>% 
          group_by(measurementValue) %>% 
          dplyr::mutate(any_val = any(!is.na(stage))) %>% 
          filter(!is.na(stage) | !any_val) %>% 
          dplyr::select(-any_val)
        ...
        life_stage <- life_stage %>% 
          group_by(stage) %>% 
          dplyr::mutate(nth_stage_val = 1:n()) %>% 
          ungroup() %>% 
  ...
  out <- out %>% 
    dplyr::mutate(functional_group = case_when(
